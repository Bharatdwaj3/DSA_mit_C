...
struct Node *deleteWithLeftChild(struct Node *root, int key){
    if(!root)
        return NULL;
    if(key<root->data)
        root->left=deleteLeftChild(root->left,key);
    if(key<root->data)
        root->right=deleteRightChild(root->right,key);
    else{
        if(root->left&&!root->right){
...
struct Node *replaceNodeWithTwoChildren(struct Node *root, int key) {
    if(!root)
        return NULL;
    if(key<root->data)
        root->left=replaceNodeWithTwoChildren(root->left,key);
    else if(key>root->data)
        root->right=replaceNodeWithTwoChildren(root->right,key);
    else{
        if(!root->left&&!root->right){
            free(root);
            return NULL;
        ...
        root->data=temp->data;
        root->right=replaceNodeWithTwoChildren(root->right,temp->data);
    }
    return root;
}
...
struct Node *replaceChildWithParent(struct Node *root, int key) {
    ...
    if(key<root->data)
        ...
        if(root->left&&!root->right){
            ...
            printf("Node with two children cannot be replaced with one parent!!");
        }
    }
    return root;
}
struct Node *deletePredessor(struct Node *root, int key) {
    if(!root)
        return NULL;
    if(key<root->data)
        root->left=deletePredessor(root->left,key);
    else if(key>root->data)
        root->right=deletePredessor(root->right,key);
    else{
        if(root->left&&root->right){
            struct Node* pred=findMax(root->left);
            root->data=pred->data;
            root->left=deletePredessor(root->left, pred->data);
        }else{
            struct Node*temp=root->left?root->left->left:root->right;
            free(root);
            return temp;
        }
    }
